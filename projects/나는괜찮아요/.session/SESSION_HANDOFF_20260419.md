...
1. **이 파일** -- 전체 맥락
2. `projects/나는괜찮아요/PROJECT.md` -- 작품 메타 + Cowork SSOT 링크 + 원문 표 13종 (섹션 1 -> 섹션 12 순)
3. `docs/workflow.md 섹션 2~3` -- 컷 단위 13단계 실전 체크리스트 (섹션 2.1 EP1 C05 예시, 섹션 2.2-B 앵커 기반 컷, 섹션 3 Kling 모션)
...
7. `C:\Team-jane\Cowork\나는괜찮아요_시즌1_프롬프트북_v3_EP1.md` 섹션 C00 섹션 -- 첫 작업 프롬프트 원문
...
- SESSION_HANDOFF_v1 L38 실측(세트 A 의상 + 골드체인 + 어깨길이 생머리)이 Cowork `캐릭터레퍼런스_v3.md 섹션 4.1`과 일치 -> 재생성 불필요
...
|---|---|---|
| `char_exlover_base_v1.png` | `캐릭터레퍼런스_v3.md 섹션 A (L22~L66)` 원문 그대로 | `assets/refs/char_exlover_base_v1.png` |
| `char_coworker_jiyeon_setA_v1.png` | `섹션 B 본문 (L70~L118)` 의상 원문 + **`L221` 헤어 정본(shoulder-length bob)으로 덮어쓰기** (L82~L84의 "medium-length wavy dark brown ... collarbone"은 폐기됨) | `assets/refs/char_coworker_jiyeon_setA_v1.png` |
| `char_coworker_jiyeon_setB_v1.png` | `섹션 B 본문 (L70~L118)` 프롬프트 골격 + **`L217` 의상 슬롯 B 스펙**(네이비 블레이저 / 흰 스트라이프 블라우스 / 베이지 슬랙스 / 블랙 플랫 로퍼 / 골드 스터드 / 실버 시계 왼손)으로 **조합** + `L221` 헤어 + `L223~L242` Clause C-EP4 참조 | `assets/refs/char_coworker_jiyeon_setB_v1.png` |

**지연 세트 B 프롬프트 조합 방법** (Cowork에 독립 프롬프트 없음):
1. `캐릭터레퍼런스_v3.md 섹션 B` 본문(L70~L118) 복사 (얼굴, 피부, 프로포션 등 공통)
...
절차는 `PROJECT.md 섹션 12 Step 1`의 공통 7단계 + `docs/workflow.md 섹션 2` 참조.

### 우선순위 2 -- 마스터 로케이션 ref 16종 (Step 2)
`PROJECT.md 섹션 4` 매트릭스 16개 로케이션별 개별 생성, 라이브러리 등록.

### 우선순위 3 -- 앵커 컷 9종 (Step 3)
**반드시 일반 Start Image보다 먼저**. `PROJECT.md 섹션 12 Step 3` 순서표 + `docs/workflow.md 섹션 2.2-B` 앵커 기반 컷 절차.

### 우선순위 4 -- EP1 Start Image 18장 (Step 4)
- C00~C17 중 **C00이 앵커**이므로 Step 3에서 처리됨
- 나머지 C01~C17은 일반/체인 대상/인서트 구분하여 순서대로
- 체인 주입 대상 (C05, C06, C07, C08, C09, C11, C12, C13) = `PROJECT.md 섹션 6.1`
...
1. **캐릭터 ref 파일명 정본**: HANDOFF v3.1 기준 `_base_v1` / `_setA_v1` / `_setB_v1`. Cowork 캐릭터레퍼런스_v3 L128의 `_ref_v1` 표기는 참고만 (PROJECT.md 섹션 3 각주).
...
4. **프레임 체이닝 fallback (섹션 7.2)**: Kling이 last->first seed 거부 시 FFmpeg로 last frame 추출 -> NanoBanana2 Reference 업로드 -> 다음 Start Image 재생성
...
| EP별 체인 앵커 번호 혼동 | EP4는 `@img4`, 나머지 `@img3` | PROJECT.md 섹션 6 매핑표 엄격 준수 |
...
2. projects/나는괜찮아요/PROJECT.md Read (섹션 1~섹션 3 우선)
3. docs/workflow.md 섹션 2 Read (컷 단위 체크리스트)
4. CHEATSHEET.md 스니펫 1로 브라우저 CDP 9222 연결, 탭 상태 실측
5. PROJECT.md 섹션 1.1 Cowork mtime 체크 (freeze 해제 여부 -- stat 5개)
...
위 명령이 **하나 이상의 커밋을 반환하면** 본 핸드오프 내용이 실제 문서와 일치하는지 대조 필수 (특히 섹션 우선순위, 섹션 핵심 의사결정, 섹션 retry-count 형식). **아무것도 반환하지 않으면** 본 핸드오프가 최신 상태.
