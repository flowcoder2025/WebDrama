...
1. **본 문서 어느 섹션이 영향받는지 체크**:
 - HANDOFF 변경 -> 섹션 1.1 읽는 순서 재확인 + 섹션 12 작업 순서 영향 점검
 - FINAL 변경 -> 섹션 2 시즌 구조, 섹션 4 로케이션, 섹션 11 품질 기준 재확인
 - 캐릭터레퍼런스 변경 -> 섹션 3 캐릭터 매트릭스 재동기화
 - 편집배포규격 변경 -> 섹션 9 발사 전 12항, 섹션 10 EP별 체크포인트 재확인
 - 프롬프트북 EP<n> 변경 -> 섹션 6 체인 주입 매핑표 재동기화
 - 공통원칙 변경 -> 섹션 7 프레임 체이닝, 섹션 8 립싱크, 섹션 12-D fallback 재확인

2. **원문 표 병합**: 본 문서의 섹션 6, 섹션 7, 섹션 8, 섹션 9 원문 표가 stale해지므로 해당 Cowork 섹션에서 신규 내용 복사

3. **버전 기록**: 섹션 14 버전 히스토리에 새 항목 추가 (예: "v3.2 2026-MM-DD -- {변경 요지}")

4. **기준 mtime 갱신**: 섹션 1.1 mtime 기록치 업데이트
...
## 3. 캐릭터 매트릭스 (HANDOFF 섹션 2 Step 1 / 캐릭터레퍼런스_v3)
...
> **파일명 정본** (2026-04-19 21:28 동기화 완료): Cowork `캐릭터레퍼런스_v3.md`도 `_base_v1` / `_setA_v1` / `_setB_v1` 형식으로 업데이트 반영됨. Cowork <-> WebDrama 1:1 매핑. `docs/standards.md 섹션 1.1 char_<id>_<slot>_v<n>.png` 패턴과 일치.
...
> **프롬프트 원문 출처**: `C:\Team-jane\Cowork\나는괜찮아요_로케이션레퍼런스_v3.md 섹션 2.1~섹션 2.16` (무인 establishing 16종 독립 프롬프트, 2026-04-19 21:10 분책).
>
> **파일명 정본** (2026-04-19 21:28 동기화 완료): Cowork `로케이션레퍼런스_v3.md 섹션 3`도 `loc_<id>_v1.png` 형식으로 업데이트 반영됨 (예: `loc_office_daytime_v1.png`). `docs/standards.md 섹션 1.1 loc_<code>_v<n>.png` 패턴과 일치, Cowork <-> WebDrama 1:1 매핑. Freepik 라이브러리 등록명은 `loc_<id>` (버전, 확장자 없이) 그대로 사용.
>
> **검수 쌍** (HANDOFF 섹션 2 Step 2 / 로케이션레퍼런스 섹션 4.7): 공간 공유 쌍 생성 후 픽셀 일치 대조 필수. 불일치 시 **두 번째만 재생성**하여 첫 번째를 앵커 기준으로 강제.
...
>
> **NanoBanana2 슬롯 규약** (로케이션레퍼런스 섹션 4.0, HANDOFF 섹션 2 Step 2):
...
### 6.1 EP1 (8 클립) -- 출처: `프롬프트북_v3_EP1.md 섹션 2.5`
| 시퀀스 | 앵커 | 체인 대상 | 주입 문구 |
|---|---|---|---|
...
### 6.2 EP2 (11 클립) -- 출처: `프롬프트북_v3_EP2.md 섹션 2.5`
| 시퀀스 | 앵커 | 체인 대상 | 주입 문구 |
|---|---|---|---|
...
### 6.3 EP3 (11 클립) -- 출처: `프롬프트북_v3_EP3.md 섹션 3.5`
| 시퀀스 | 앵커 | 체인 대상 | 주입 문구 |
|---|---|---|---|
...
### 6.4 EP4 (9 클립) -- 출처: `프롬프트북_v3_EP4.md 섹션 3.5` -- **앵커는 `@img4`**
| 시퀀스 | 앵커 | 체인 대상 | 주입 문구 |
|---|---|---|---|
...
Kling `last->first seed` 체이닝 강제 대상 (Cowork 공통원칙 섹션 12-B):
...
### 7.1 Kling 프롬프트 부가 라인 (공통원칙 섹션 12-C)
...
### 7.2 **"Kling이 last->first seed 거부" fallback 실행 절차 (공통원칙 섹션 12-D)**
...
5. **2회 실패 시**: 두 클립 사이에 **0.3s 블랙 컷** 또는 **인서트 샷** 배치 (편집 규약 -- `편집배포규격_v3.md 섹션 1.1 크로스 페이드 기본 6 프레임` 참조)
...
## 8. 립싱크 예외 3컷 (HANDOFF 섹션 2 Step 5 / FINAL 섹션 8)
...
| ref | 상태 | 절차 |
|---|---|---|
| `char_eunseo_base_v1.png` | [OK] 재활용 완료 (`assets/refs/`에 복사됨) | Freepik Reference 라이브러리에 업로드만 남음 |
| `char_exlover_base_v1.png` | 신규 생성 필요 | Cowork `캐릭터레퍼런스_v3.md 섹션 A (L22~L66)` 프롬프트 사용 -> NanoBanana2 호출 -> 2K 저장 -> Freepik 라이브러리 등록 |
| `char_coworker_jiyeon_setA_v1.png` | 신규 (EP1 배경용) | Cowork `섹션 B (L70~L118) 세트 A` 프롬프트 사용 |
| `char_coworker_jiyeon_setB_v1.png` | 신규 (EP4 face-on) | Cowork `섹션 B 세트 B` 프롬프트 사용 |

각 신규 ref 생성 절차 (공통):
1. Cowork 캐릭터레퍼런스 파일에서 해당 섹션 의 프롬프트 전체 복사
...
FINAL 섹션 5 매트릭스 기준. 각 로케이션별 개별 생성, 라이브러리 등록. 파일명: `loc_<code>_v1.png` (예: `loc_office_daytime_v1.png`). 저장: `assets/refs/` (캐릭터 ref와 동일 디렉토리, prefix로 구분).
...
컷별 절차는 `docs/workflow.md 섹션 2 "컷 단위 실전 체크리스트"` 참조. 체인 대상 클립은 섹션 6의 매핑 표에 따라 `@img3` 또는 `@img4` 멘션 **최상단 삽입**.
...
- **프레임 체이닝 8쌍**: 섹션 7 실행 절차 (`last->first seed` 또는 `섹션 12-D fallback`)
- **립싱크 예외 3컷**: 섹션 8 phoneme 시도 (실패 시 VO fallback)
...
- SFX: 12종 라이브러리 또는 CC0 (편집배포규격 섹션 9.2)
- **착수 시점**: Step 6 완료 후
